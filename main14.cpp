...
int cmp(Population a,Population b){
	...
	return (abs(a.bestgene.fitness - a.meanFit) / abs(a.bestgene.fitness - a.worstgene.fitness)) < (abs(b.bestgene.fitness - b.meanFit) / abs(b.bestgene.fitness - b.worstgene.fitness));  
}
	...
	for(int times = 1;times <= ALL_TIMES;times++){
		...
		for(int i = 0;i < subPop;i++){
			if(i % 3 == 0)
				...
				pop[i] = Population(num,crossRate3,scale,arrayLow,arrayUpper,func);
		}
