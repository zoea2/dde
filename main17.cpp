...
void updateScaleAndCrRate(Population& population){
	const double Fl = 0.1;
	const double Fu = 0.9;
	const double updateRate = 0.1;
	for(int i = 0;i < population.num;i++){
		if(randomNumber() <= updateRate)
			population.genes[i].genoScale = Fl + randomNumber() * Fu;
		if(randomNumber() <= updateRate)
			population.genes[i].genoCrRate = randomNumber();
	}
}
